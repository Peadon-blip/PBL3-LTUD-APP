...
-- Roles
INSERT INTO ROLES (role_name) VALUES (N'Admin'), (N'Student');
...
INSERT INTO DIEU_KIEN_TIEN_QUYET (id_mon_hoc, id_mon_tq)
...
-- Giảng viên
INSERT INTO GIANG_VIEN (ten_giang_vien, email, khoa, rating) VALUES
...
-- Phòng học
INSERT INTO DANH_MUC_PHONG (ten_phong, toa_nha, loai_phong, suc_chua) VALUES
('A101', N'Nhà A', 'LT', 60), ('A102', N'Nhà A', 'LT', 60),
('B201', N'Nhà B', 'TH', 40), ('B202', N'Nhà B', 'TH', 40),
('C301', N'Nhà C', 'LT', 80), ('C302', N'Nhà C', 'TH', 40);

-- Lớp học phần HK1 2024-2025
DECLARE @gv1 BIGINT = (SELECT TOP 1 id_giang_vien FROM GIANG_VIEN WHERE ten_giang_vien LIKE N'%Hùng%');
DECLARE @gv2 BIGINT = (SELECT TOP 1 id_giang_vien FROM GIANG_VIEN WHERE ten_giang_vien LIKE N'%Lan%');
DECLARE @gv3 BIGINT = (SELECT TOP 1 id_giang_vien FROM GIANG_VIEN WHERE ten_giang_vien LIKE N'%Minh%');
DECLARE @gv4 BIGINT = (SELECT TOP 1 id_giang_vien FROM GIANG_VIEN WHERE ten_giang_vien LIKE N'%Hoa%');
DECLARE @p1  BIGINT = (SELECT TOP 1 id_phong FROM DANH_MUC_PHONG WHERE ten_phong='A101');
DECLARE @p2  BIGINT = (SELECT TOP 1 id_phong FROM DANH_MUC_PHONG WHERE ten_phong='B201');
DECLARE @p3  BIGINT = (SELECT TOP 1 id_phong FROM DANH_MUC_PHONG WHERE ten_phong='C301');

INSERT INTO LOP_HOC_PHAN (id_mon_hoc, id_giang_vien, id_phong, hoc_ky, nam_hoc)
SELECT id_mon_hoc,
